...
| **LLM** | OpenRouter API · primary: `poolside/laguna-m.1:free` (set via `OPENROUTER_MODEL` Vercel env) · fallback chain: Gemma 4 31B → Nemotron 3 Super 120B → Llama 3.3 70B | On-demand via Next.js API route (`/api/narratives/[reep_id]`). Planned PR7: nightly pre-gen (`generate_narratives.py`) with templated anti-hallucination pattern — Python emits structured fact bullets, LLM only rephrases. Vercel `maxDuration = 60` required; Laguna M.1 is reasoning-tuned so TTFT exceeds default 10s Hobby cap. |
...
| 11 | **Reasoning-model latency vs Vercel timeout** — Poolside Laguna M.1 and other reasoning models (Nemotron Ultra, DeepSeek R1) emit a hidden `<think>` preamble before answering; time-to-first-token routinely exceeds Vercel Hobby's 10s function cap, producing 504/500 errors. | Route sets `maxDuration = 60` (requires Vercel Pro); strips `<think>…</think>` and `<reasoning>…</reasoning>` from output; `max_tokens` raised to 800 to give reasoning headroom. Long-term fix: nightly pre-generation (PR7) makes runtime calls rare. Fallback chain (Gemma 4 31B → Nemotron 3 Super → Llama 3.3 70B) ensures the route never returns empty on a single-model failure. |
