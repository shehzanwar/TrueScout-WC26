...
![Live Demo](https://img.shields.io/badge/Live_Demo-truescout.vercel.app-000?logo=vercel&logoColor=white)
![Next.js](https://img.shields.io/badge/Next.js-16-black?logo=next.js&logoColor=white)
![Python](https://img.shields.io/badge/Python-3.11+-3776AB?logo=python&logoColor=white)
![Vercel](https://img.shields.io/badge/Deployed_on-Vercel-000)
![GitHub Actions](https://img.shields.io/badge/Pipeline-GitHub_Actions-2088FF?logo=githubactions&logoColor=white)
</div>
---
📖 Overview
...
"How good is this player really?" — raw numbers lie because a 20-goal season in a weak league is not the same as 20 goals in a top-5 European league. TrueScout translates two years of club form into a single, league-adjusted, position-scoped rating using a hierarchical Bayesian model, then updates it with World Cup minutes as fresh evidence.
"Who's actually going to win?" — it re-simulates the remaining bracket 100,000 times every matchday to produce calibrated title and advancement probabilities, and grades itself against the betting market with a running Brier score.
...
A Google AI API key (aistudio.google.com/apikey) — required for on-demand scouting reports
1. Clone the repository
```bash
git clone https://github.com/shehzanwar/TrueScout-WC26.git
cd TrueScout-WC26
```
...
---
☁️ Deployment
TrueScout is designed for zero-cost hosting: static JSON committed to the repo + a Next.js app on Vercel + a scheduled GitHub Actions runner. Full step-by-step instructions live in `DEPLOYMENT.md`.
Vercel (frontend)
Import the repo at vercel.com → Add New Project.
Set Framework Preset → `Next.js`, Root Directory → `frontend`.
Add `GOOGLE_AI_API_KEY` under Project Settings → Environment Variables.
Deploy — the placeholder JSONs work on first run.
...
In the repo → Settings → Actions → General, ensure "Allow all actions" is on.
`GITHUB_TOKEN` is auto-provisioned — no manual secret needed.
Trigger the first run via Actions → Nightly ETL + Static Export → Run workflow.
...
---
🗺️ Roadmap
A full Phase 5 queue lives in `BOARD.md`. Highlights:
...
📚 Documentation
Doc	What it covers
`PRD.md`	Product scope, success metrics, user flow, in/out-of-scope
`ARCHITECTURE.md`	Tech stack, data model, third-party APIs, risk register
`DEPLOYMENT.md`	Vercel + GitHub Actions setup, local dev, failure modes
`BOARD.md`	Roadmap phases and backlog
`CONTEXT.md`	Living project context
---
🤝 Contributing
This is a personal portfolio project, but suggestions and issue reports are welcome.
Fork the repository and create a feature branch (`feat/your-idea`).
Keep changes within the scope defined in `PRD.md` — if it isn't in the PRD, it isn't being built yet.
Run the audit scripts under `etl/audits/` to catch data-quality regressions.
Open a pull request describing the change and linking any relevant issue.
---
👤 Author & Acknowledgements
Shehzad Anwar — github.com/shehzanwar
